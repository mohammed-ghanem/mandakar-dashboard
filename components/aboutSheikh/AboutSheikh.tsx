...
} from "@/store/settings/aboutSheikhApi";
...
export default function AboutSheikh() {
  ...
  const t = translate?.settings?.aboutSheikh;

  const { data, isLoading, isError } = useGetAboutSheikhQuery(undefined, {
    skip: !sessionReady,
  });
  const [updateAboutSheikh, { isLoading: isSaving }] =
    useUpdateAboutSheikhMutation();
  ...
  const submit = async () => {
    try {
      const res = await updateAboutSheikh(form).unwrap();
