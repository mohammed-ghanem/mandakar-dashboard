...
import { ISpeechLink } from "@/types/speeches";
...
export default function EditSpeech() {
  ...
  const t = translate?.pages.speeches?.editSpeech;
  ...
  const [updateSpeech, { isLoading: isUpdating }] =
    useUpdateSpeechMutation();
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      content_ar: speech.content?.ar ?? "",
      content_en: speech.content?.en ?? "",
      category_id:
        speech.category_id != null ? String(speech.category_id) : "",
      ...
      attachmentRows: [{ key: newKey(), file: null }],
      links:
        speech.links && speech.links.length > 0
          ? speech.links
          : [{ title: "", url: "" }],
      seo_description: speech.seo?.description ?? "",
      seo_keywords: (speech.seo?.keywords ?? []).join(", "),
    });
  ...
  const existingAttachments = speech.attachments ?? [];
        ...
        <CardContent className={dash.formCardContent}>
          <form onSubmit={submit} className="space-y-8 md:space-y-10">
            ...
            <section className={dash.sectionNeutral}>
              ...
              <ImageDropzone
                file={form.image}
                existingImageUrl={speech.image || undefined}
            ...
            <section className={dash.sectionNeutral}>
              ...
              <AudioDropzone
                file={form.audio}
                existingAudioUrl={speech.audio || undefined}
